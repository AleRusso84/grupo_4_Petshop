...
function ContentRowMarkPet(){

    const [products, setProducts] = useState([]);

    const obtenerDatosProducts = async ()=>{
        const data = await  fetch('http://localhost:3030/product/apiview')
    ...
    const obtenerDatosUsers = async ()=>{
        const data = await  fetch('http://localhost:3030/users/apiview')
...
const [category, setCategory] = useState([]);
  ...
  const obtenerDatosCategory = async ()=>{
      const data = await  fetch('http://localhost:3030/category/apiview')
...
export default ContentRowMarkPet;